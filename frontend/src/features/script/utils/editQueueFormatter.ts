...
export class EditQueueFormatter {
  ...
  static formatOperation(
    ...
    const elementName =
      ...
      `Element ${operation.element_id?.slice(-6) || "Unknown"}`;
    ...
    switch (operation.type) {
      ...
      case "UPDATE_TIME_OFFSET":
        const oldTime = formatTimeOffset(
          operation.old_offset_ms,
          useMilitaryTime,
        );
        const newTime = formatTimeOffset(
          operation.new_offset_ms,
          useMilitaryTime,
        );
      ...
      case "CREATE_ELEMENT":
        const elementType =
          operation.element_data.element_type?.toLowerCase() || "element";
        const insertText =
          operation.insert_index !== undefined
            ? ` at position ${operation.insert_index + 1}`
            : "";
      ...
      case "UPDATE_ELEMENT":
        return this.formatElementUpdate(
          operation,
          elementName,
          useMilitaryTime,
        );
   ...
   */
  private static formatScriptInfoUpdate(
    operation: any,
    useMilitaryTime: boolean = false,
  ): string {
    ...
    if (changedFields.length === 1) {
      ...
      const oldValue = this.formatScriptValue(
        field,
        change.old_value,
        useMilitaryTime,
      );
      const newValue = this.formatScriptValue(
        field,
        change.new_value,
        useMilitaryTime,
      );
   ...
   */
  private static formatElementUpdate(
    operation: any,
    elementName: string,
    _useMilitaryTime: boolean = false,
  ): string {
    const changes = operation.changes || {};
    const changedFields = Object.keys(changes);
    ...
    if (changedFields.length === 1) {
      ...
      const oldValue = this.formatValue(
        field,
        change.old_value ?? change.oldValue,
      );
      const newValue = this.formatValue(
        field,
        change.new_value ?? change.newValue,
      );
    ...
    const fieldNames = changedFields
      .map((field) => fieldDisplayNames[field] || this.formatFieldName(field))
      .join(", ");
   ...
   */
  private static formatGroupUpdate(
    operation: any,
    elementName: string,
    useMilitaryTime: boolean = false,
  ): string {
    ...
    if (changedFields.length === 1) {
      ...
      if (field === "offset_ms" && affectedChildren.length > 0) {
        ...
        const timeChange =
          offsetDelta > 0
            ? `+${formatTimeOffset(Math.abs(offsetDelta), useMilitaryTime)}`
            : `-${formatTimeOffset(Math.abs(offsetDelta), useMilitaryTime)}`;
        return `Updated group "${elementName}" ${fieldName} (${timeChange}, ${affectedChildren.length} children affected)`;
      }

      const formattedOldValue = this.formatValue(
        field,
        oldValue,
        useMilitaryTime,
      );
      const formattedNewValue = this.formatValue(
        field,
        newValue,
        useMilitaryTime,
      );
    ...
    const childrenNote =
      affectedChildren.length > 0
        ? ` (${affectedChildren.length} children affected)`
        : "";
   ...
   */
  private static formatScriptValue(
    field: string,
    value: any,
    useMilitaryTime: boolean = false,
  ): string {
   ...
   */
  private static formatFieldUpdate(
    operation: any,
    elementName: string,
    _useMilitaryTime: boolean = false,
  ): string {
  ...
  static formatTimestamp(
    ...
    if (date.toDateString() === today.toDateString()) {
      ...
      const time = formatAbsoluteTimeStandard(
        date.toISOString(),
        useMilitaryTime,
      );
  ...
  static formatOperationsSummary(operations: EditOperation[]): string {
    ...
    const parts = Object.entries(types).map(([type, count]) => {
      const desc =
        typeDescriptions[type as keyof typeof typeDescriptions] || "changes";
      return `${count} ${desc}`;
    });
