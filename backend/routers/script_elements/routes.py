...
@router.get("/scripts/{script_id}/elements", response_model=Union[List[schemas.ScriptElement], schemas.SharedScriptElementsResponse])
def get_script_elements(
    ...
    logger.error(f"❌❌❌ SEPARATE ELEMENTS ENDPOINT CALLED - script {script_id} - should use unified endpoint instead!")  
...
def get_script_element(
    ...
    logger.error(f"❌❌❌ SEPARATE ELEMENT ENDPOINT CALLED - element {element_id} - should use unified endpoint instead!")
