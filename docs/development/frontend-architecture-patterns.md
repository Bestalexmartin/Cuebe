...
const useModalManager = () => {
  ...
  return {
    openModal,
    closeModal,
    isOpen: (name: string) => state.openModals.has(name),
  };
};
```
...
const useScript = (scriptId: string) =>
  useResource<Script>(`/api/scripts/${scriptId}`, [scriptId]);
```
...
const useScriptElements = (scriptId: string) => {
  ...
  const updateElement = useCallback(
    (elementId: string, changes: any) => {
      // Implementation
    },
    [scriptId],
  ); // Only recreate if scriptId changes
  ...
  useEffect(() => {
    ...
    if (process.env.NODE_ENV === "development") {
      if (renderCount.current > 10) {
        console.warn(
          `${componentName} has rendered ${renderCount.current} times - check for optimization opportunities`,
        );
      }
    }
  });
